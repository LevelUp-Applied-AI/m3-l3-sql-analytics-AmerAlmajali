...
SELECT DISTINCT
    ...
    e.last_name
FROM employees e
JOIN project_assignments pa1 
    ON e.employee_id = pa1.employee_id
JOIN project_assignments pa2 
    ON pa1.project_id = pa2.project_id
JOIN employees e2 
    ON pa2.employee_id = e2.employee_id
...
DECLARE
    ...
    active_proj INT;
BEGIN
    SELECT COUNT(*) INTO emp_count FROM employees e JOIN departments d ON e.department_id = d.department_id WHERE d.name = dept_name;
    SELECT SUM(e.salary) INTO total_sal FROM employees e JOIN departments d ON e.department_id = d.department_id WHERE d.name = dept_name;
    SELECT COUNT(DISTINCT pa.project_id) INTO active_proj FROM project_assignments pa JOIN employees e ON pa.employee_id = e.employee_id JOIN departments d ON e.department_id = d.department_id WHERE d.name = dept_name;
...
SELECT employee_id, salary, CURRENT_DATE - INTERVAL '1 year', NULL
FROM employees;


-- Generate some historical salary records
INSERT INTO salary_history (employee_id, salary, start_date, end_date)
SELECT employee_id, salary * 0.95, CURRENT_DATE - INTERVAL '2 year', CURRENT_DATE - INTERVAL '1 year'
FROM employees;

INSERT INTO salary_history (employee_id, salary, start_date, end_date)
SELECT employee_id, salary * 0.90, CURRENT_DATE - INTERVAL '3 year', CURRENT_DATE - INTERVAL '2 year'
FROM employees;


SELECT 
    d.name AS department_name,
    EXTRACT(YEAR FROM sh.start_date) AS year,
    ROUND(AVG(sh.salary), 2) AS avg_salary,
    ROUND((AVG(sh.salary) - LAG(AVG(sh.salary)) OVER (PARTITION BY d.department_id ORDER BY EXTRACT(YEAR FROM sh.start_date))) / LAG(AVG(sh.salary)) OVER (PARTITION BY d.department_id ORDER BY EXTRACT(YEAR FROM sh.start_date)) * 100, 2) AS growth_percent
FROM salary_history sh
JOIN employees e ON sh.employee_id = e.employee_id
JOIN departments d ON e.department_id = d.department_id
GROUP BY d.department_id, d.name, year
